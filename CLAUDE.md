...
Web UIでは「リサーチ結果」タブがメイン画面で、Note向けに整形された記事文体ではなく、ユーザー本人がそのまま読んでわかりやすい平易な文章でまとめられる。記事下書き・動画構成は、そのリサーチ結果をもとにした派生成果物という位置づけ。生成物ページには「コピー」ボタンもあり、今開いているタブ（リサーチ結果/記事下書き/動画構成）の本文をMarkdownのままクリップボードにコピーできる（他アプリへの転記用。サーバー側の状態は変更しないクライアントのみの機能）。
...
`revisionRequested`（真偽値。デフォルト`false`）と`revisionNote`（文字列または`null`）は、生成物ページの「修正をリクエスト」ボタン（どのタブを開いていても表示される）で設定される。ユーザーが何を直してほしいか一言書いて送ると`revisionRequested: true`・`revisionNote`にその文章が入る。次回のパイプライン実行で、`status`や頻度スケジュールとは無関係に処理対象へ加わり、既存のリサーチ内容を活かしつつ修正リクエストに沿って狙い撃ちで再調査される（詳細は下記「修正リクエスト」参照）。処理が終わると`revisionRequested`は`false`に戻る。
...
- 仕組み: 生成物ページに「修正をリクエスト」ボタンがある（記事下書き/動画構成タブを開いていても表示される — 常にそのメモのリサーチ結果に対する修正リクエストとして扱われる）。押すと何を直してほしいか一言入力するプロンプトが出て、送信すると`revisionRequested: true`・`revisionNote: <入力内容>`になる（`PUT /api/memos/<id>`経由）。次回の`/research-pipeline`実行で、そのメモの`status`や定期メモの頻度スケジュールとは無関係にこの修正が処理される。
