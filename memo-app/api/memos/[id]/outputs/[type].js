...
export default async function handler(req, res) {
  ...
  if (req.method === "PUT") {
    // Vercel only auto-parses the body to a string for an exact
    // "text/plain" Content-Type; other text/* types (e.g. text/markdown)
    // arrive as a raw Buffer, so handle both.
    const body = typeof req.body === "string" ? req.body : Buffer.isBuffer(req.body) ? req.body.toString("utf-8") : "";
    if (!body.trim()) {
